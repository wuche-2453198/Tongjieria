...
struct TransformComponent
{
  ...
  cocos2d::Vec2 previousPosition = cocos2d::Vec2::ZERO;  // 上一帧位置（用于插值和碰撞检测）
  ...
  TransformComponent(float x, float y) : position(x, y), previousPosition(x, y) {}
  TransformComponent(const cocos2d::Vec2 &pos) : position(pos), previousPosition(pos) {}
};
