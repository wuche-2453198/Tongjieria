...
entt::entity PlayerFactory::createPlayer(entt::registry& registry,
    ...
    transform.previousPosition = spawnPos;
    ...
    // Keep a HealthComponent for combat/collision systems
    auto& health = registry.emplace<ecs::HealthComponent>(player);
    health.maxHealth = stats.maxHealth;
    health.currentHealth = stats.currentHealth;
    health.invincibleTime = stats.invincibleDuration;

    // Block systems expect Position/LoadingTicket for actors
    registry.emplace<Position>(player, spawnPos);
    registry.emplace<LoadingTicket>(player, player, 8, false);
    ...
    if (sprite) {
        ...
        for (auto state : allStates) {
            ...
            for (auto* frame : anim->frames) {
                ...
                if (frame->getParent() != sprite) {
                    frame->removeFromParent();
                    sprite->addChild(frame, 1);
                }
    ...
    // Register for unified physics/transform sync systems (NpcBlockTicketSyncSystem / PhysicsSyncSystemEntt)
    if (sprite) {
        auto& state = registry.emplace<ecs::SpriteStateComponent>(player);
        state.spriteCreated = true;
        state.spriteHandle = sprite;

        auto& physics = registry.emplace<ecs::PhysicsBodyComponent>(player);
        physics.shape = ecs::PhysicsBodyComponent::BodyShape::Box;
        {
            Size spriteSize = sprite->getContentSize();
            float bodyWidth = spriteSize.width * 0.6f;
            float bodyHeight = spriteSize.height * 0.8f;
            if (spriteSize.width <= 0 || spriteSize.height <= 0) {
                bodyWidth = 20.0f;
                bodyHeight = 42.0f;
            }
            physics.width = bodyWidth;
            physics.height = bodyHeight;
        }
        physics.offset = cocos2d::Vec2::ZERO;
        ...
        physics.categoryBitmask = 0x0008;
        physics.contactTestBitmask = 0xFFFFFFFF;
        physics.collisionBitmask = 0x0001 | 0x0002;
        physics.group = 0;
        physics.needsCreation = false;
    }

    if (sprite) {
        ecs::NodeEntityMap::getInstance().registerNode(sprite, entt::to_integral(player));
    }
...
Sprite* PlayerFactory::createPlayerSprite(const Vec2& spawnPos, Node* parentNode) {
    ...
    if (sprite) {
        ...
        sprite->setOpacity(0);
        sprite->setCascadeOpacityEnabled(false);
...
void PlayerFactory::addPhysicsBody(Sprite* sprite) {
    ...
    if (physicsBody) {
        ...
        physicsBody->setCollisionBitmask(0x0001 | 0x0002);
        physicsBody->setCategoryBitmask(0x0008);       // Player category
