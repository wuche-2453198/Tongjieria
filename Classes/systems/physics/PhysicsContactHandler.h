...
public:
    ...
    static constexpr float GROUND_NORMAL_X_MAX = 0.3f;
    static constexpr float WALL_NORMAL_X_THRESHOLD = 0.7f;   // 墙壁法线X阈值
    static constexpr float WALL_NORMAL_Y_THRESHOLD = 0.8f;   // 墙壁法线Y阈值
    ...
    static ContactInfo parseContact(cocos2d::PhysicsContact& contact) {
        ...
        if (auto* data = contact.getContactData(); data && data->count > 0) {
            info.point = data->points[0];
            info.hasPoint = true;
        }
        
        // 判断接触类型
        if (info.hasPoint && info.dynamicNode) {
            const float dy = info.point.y - info.dynamicNode->getPositionY();
            const float dx = info.point.x - info.dynamicNode->getPositionX();
            const bool isBelowCenter = (dy < -0.5f);
            const float nx = std::abs(info.normal.x);
            const float ny = std::abs(info.normal.y);
            info.isGroundContact = isBelowCenter && (ny > 0.8f) && (nx < GROUND_NORMAL_X_MAX);

            const bool isSideContact = (std::abs(dx) > std::abs(dy)) && (std::abs(dx) > 0.5f);
            const bool wallByNormal = (nx > WALL_NORMAL_X_THRESHOLD) && (ny < WALL_NORMAL_Y_THRESHOLD);
            const bool wallByPoint = isSideContact && (nx > 0.3f) && (ny < 0.95f);
            info.isWallContact = wallByNormal || wallByPoint;
        } else {
            info.isGroundContact = (std::abs(info.normal.y) > 0.8f) &&
    ...
    static void handleWallPreSolve(const ContactInfo& info, cocos2d::PhysicsContactPreSolve& solve) {
        ...
        if (info.dynamicBody) {
            auto v = info.dynamicBody->getVelocity();

            if (std::abs(info.normal.x) >= 0.3f) {
                const float pushDir = info.normal.x;
                if ((pushDir > 0.0f && v.x < 0.0f) || (pushDir < 0.0f && v.x > 0.0f)) {
                    v.x = 0.0f;
                    info.dynamicBody->setVelocity(v);
                }
            }
        }
    }
