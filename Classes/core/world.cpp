...
#include "game scene/NewGameScene.h"
...
bool World::init() 
{
...

	{
		...
		listener->onKeyPressed = [this](cocos2d::EventKeyboard::KeyCode keyCode, cocos2d::Event*) {
			...
			else if (keyCode == cocos2d::EventKeyboard::KeyCode::KEY_DELETE) {
				// 按delete键切换到 NewGameScene 并打开设置
				switchToNewGameSceneWithSetting();
			}
		};
...
void World::switchToNewGameSceneWithSetting()
{
	// 停止所有更新和计时器
	unscheduleUpdate();

	// 创建场景切换效果
	auto transition = cocos2d::TransitionFade::create(0.5f, NewGameScene::createScene());

	// 使用 UserDefault 传递参数，表示要打开设置
	cocos2d::UserDefault::getInstance()->setBoolForKey("open_setting_from_world", true);
...
void World::initSaveSystem()
{
	_currentSaveSlot = cocos2d::UserDefault::getInstance()->getIntegerForKey("load_slot", 1);
	...
	if (SimpleSaveManager::getInstance()->loadGameWithEntt(
		...
		if (auto* transform = _registry->try_get<ecs::TransformComponent>(_playerEntity)) {
			if (auto* cam = getDefaultCamera()) {
				cam->setPosition(cocos2d::Vec2(transform->position.x, transform->position.y));
				transform->position = cam->getPosition();
			}
		}

		CCLOG("World: Save loaded from slot %d", _currentSaveSlot);
		return true;
	}
...
void World::createNewGame()
{
	// 设置默认存档数据
	_currentSaveData.slotNumber = _currentSaveSlot;
	_currentSaveData.saveName = "New World";
	_currentSaveData.saveTime = time(nullptr);

	// 设置玩家初始位置
	if (auto* cam = getDefaultCamera()) {
		_currentSaveData.playerPos.position.x = cam->getPosition().x;
		_currentSaveData.playerPos.position.y = cam->getPosition().y;
	}

	// 应用到 ECS
	if (_playerEntity != entt::null) {
		if (auto* transform = _registry->try_get<ecs::TransformComponent>(_playerEntity)) {
			transform->position = cocos2d::Vec2(_currentSaveData.playerPos.position.x,
				_currentSaveData.playerPos.position.y);
		}

		if (auto* health = _registry->try_get<ecs::HealthComponent>(_playerEntity)) {
			// 如果需要，可以设置血量等
		}
	}
...
void World::saveCurrentGame()
{
	...
	if (_playerEntity != entt::null) {
		if (auto* transform = _registry->try_get<ecs::TransformComponent>(_playerEntity)) {
			_currentSaveData.playerPos.position.x = transform->position.x;
			_currentSaveData.playerPos.position.y = transform->position.y;
		}
	}
	...
	if (SimpleSaveManager::getInstance()->saveGameWithEntt(
		...
		CCLOG("World: Game saved to slot %d", _currentSaveSlot);
	}
	else {
...
void World::showSaveNotification(const std::string& message)
{
	// 移除现有的通知
	if (_saveNotification) {
		_saveNotification->removeFromParent();
		_saveNotification = nullptr;
	}

	auto visibleSize = cocos2d::Director::getInstance()->getVisibleSize();
	_saveNotification = cocos2d::Label::createWithSystemFont(message, "Arial", 24);
	_saveNotification->setPosition(cocos2d::Vec2(visibleSize.width / 2, 100));
	_saveNotification->setColor(cocos2d::Color3B::GREEN);
	addChild(_saveNotification, std::numeric_limits<int>::max());
	...
	_saveNotification->runAction(cocos2d::Sequence::create(
