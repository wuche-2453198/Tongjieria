...
bool CraftBar::init() {
    ...
    // CRITICAL: Enable update to keep crafting bar fixed on screen
    this->scheduleUpdate();
...
void CraftBar::update(float dt) {
    ...
    // Calculate craft bar position relative to camera
    // CraftBar is at bottom-left of screen with margin
    float marginLeft = 20.0f;
    float marginBottom = 70.0f;

    // Position = camera position + offset from camera center to screen bottom-left + margins
    float posX = camPos.x - (visibleSize.width / 2.0f) + marginLeft;
    float posY = camPos.y - (visibleSize.height / 2.0f) + marginBottom;

    setPosition(Vec2(posX, posY));

    // 统一更新展开的合成面板位置（跟随相机）
    if (_expandedCraftingPanel) {
        _expandedCraftingPanel->setPosition(Vec2(camPos.x, camPos.y));
    }

    // 统一更新快速合成栏位置（跟随相机）
    if (_quickCraftingBar) {
        float barY = camPos.y + visibleSize.height * 0.15f - 70.0f;  // 相对于相机中心的偏移
        _quickCraftingBar->setPosition(Vec2(camPos.x, barY));
    }
}
...
void CraftBar::createQuickCraftingBar() {
    ...
    if (scene) {
        ...
        CCLOG("CraftBar: Quick crafting bar created (position will follow camera)");
    }
}
