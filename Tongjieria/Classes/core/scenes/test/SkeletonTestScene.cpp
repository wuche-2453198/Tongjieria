...
Scene *SkeletonTestScene::createScene() {
  ...
  auto layer = SkeletonTestScene::create();
  scene->addChild(layer);
  return scene;
}

bool SkeletonTestScene::init() {
  if (!Layer::init()) {
    return false;
  }
  ...
  auto background = LayerColor::create(Color4B(20, 20, 20, 255));
  this->addChild(background, 0);

  auto titleLabel = Label::createWithTTF("Skeleton Test Scene", "fonts/Marker Felt.ttf", 32);
  ...
  auto infoLabel = Label::createWithTTF(
      "Skeletons: Warrior AI; Angry Bones pounce when near/above (WASD move player, R toggle)",
  ...
  auto backItem = MenuItemLabel::create(backLabel, CC_CALLBACK_1(SkeletonTestScene::menuBackCallback, this));
...
void SkeletonTestScene::setupEcsSystems() {
  ...
  _systemManager.addSystem<ecs::AngryBonesAISystemEntt>();
...
void SkeletonTestScene::createPhysicsEnvironment() {
  auto visibleSize = Director::getInstance()->getVisibleSize();
  Vec2 origin = Director::getInstance()->getVisibleOrigin();
  ...
  leftWall->setColor(Color3B(50, 50, 60));
  ...
  rightWall->setColor(Color3B(50, 50, 60));
  ...
  topWall->setColor(Color3B(50, 50, 60));
  ...
  ground->setColor(Color3B(60, 60, 70));
...
void SkeletonTestScene::createFakePlayerEntity() {
  auto visibleSize = Director::getInstance()->getVisibleSize();
  Vec2 origin = Director::getInstance()->getVisibleOrigin();
...
void SkeletonTestScene::createSkeletons() {
  auto visibleSize = Director::getInstance()->getVisibleSize();
  Vec2 origin = Director::getInstance()->getVisibleOrigin();
  ...
  for (int i = 0; i < count; i++) {
    ...
    float y = origin.y + 80.0f;
    factory.createMonster(_registry, ids[i], x, y, this);
  }
}

void SkeletonTestScene::setupSharedContactListener() {
...
void SkeletonTestScene::setupKeyboardListener() {
...
void SkeletonTestScene::update(float delta) {
  Layer::update(delta);
  updateFakePlayerPosition(delta);
  _systemManager.update(delta);
}

void SkeletonTestScene::updateFakePlayerPosition(float delta) {
...
void SkeletonTestScene::toggleFakePlayer() {
...
void SkeletonTestScene::menuBackCallback(Ref *pSender) {
