...
当前合成系统有被加入到playersystem中了吗
你现在找到合成系统，将合成系统整合到Classes\IntegrationTestScene.cpp中同时，UI界面的刷新也需要进行改变，原理需要与物品系统UI显示，状态栏系统UI显示相同，

你现在检查playersystem中挖掘逻辑，我先需要你调用以下的这些逻辑，内容包括有先删去现有的挖矿逻辑，
在挖矿逻辑处进行修改不去调用trymine，而是去调用tryDestory，

将合成功能整合到D:\test\Terraria_tj\Classes\IntegrationTestScene.cpp他应该与物品栏部分的UI显示逻辑相同，同时，
